...
async def test_bridge_forward_redis_messages_success(bridge):
    ...
    async def get_msg_side_effect(*args, **kwargs):
        if get_msg_side_effect.calls == 0:
            ...
            return {"type": "message", "channel": "ch1", "data": json.dumps({"foo": "bar"})}
    ...
    with patch("src.python.agents.dashboard_bridge.aioredis.from_url", AsyncMock(return_value=bridge.redis)):
...
async def test_bridge_broadcast_system_stats_success(bridge):
    ...
    async def sleep_side_effect(*args, **kwargs):
        nonlocal call_count
        call_count += 1
        if call_count > 1: bridge.running = False
        return None
...
async def test_bridge_run(bridge):
    ...
    with patch("src.python.agents.dashboard_bridge.websockets.serve", return_value=AsyncMock()) as mock_ws:
...
async def test_bridge_main_config_error():
    ...
    with patch("src.python.agents.dashboard_bridge.open", m), \
         patch("src.python.agents.dashboard_bridge.yaml.safe_load", side_effect=Exception("load error")), \
...
async def test_bridge_main_keyboard_interrupt():
    ...
    with patch("src.python.agents.dashboard_bridge.open", m), \
         patch("src.python.agents.dashboard_bridge.validate_config", return_value=(True, "")), \
         patch("src.python.agents.dashboard_bridge.DashboardBridge.run", side_effect=KeyboardInterrupt), \
         patch("src.python.agents.dashboard_bridge.DashboardBridge.stop", return_value=None) as mock_stop:
        await bridge_main()
...
async def test_bridge_main_validation_error():
    m = mock_open(read_data="system: {environment: paper}")
    with patch("src.python.agents.dashboard_bridge.open", m), \
         patch("src.python.agents.dashboard_bridge.yaml.safe_load", return_value={"system": {}}), \
         patch("src.python.agents.dashboard_bridge.validate_config", return_value=(False, "invalid")), \
         patch("sys.exit", side_effect=SystemExit(1)) as mock_exit:
...
async def test_bridge_forward_redis_messages_error(bridge):
    ...
    async def get_msg_side_effect(*args, **kwargs):
        ...
        if get_msg_side_effect.calls == 1:
            get_msg_side_effect.calls += 1
            return {"type": "message", "channel": "ch1", "data": json.dumps({"error": "trigger"})}
        bridge.running = False
        return None
    get_msg_side_effect.calls = 0
    bridge.pubsub.get_message = AsyncMock(side_effect=get_msg_side_effect)
    ...
    def mock_loads(s):
        if "trigger" in s: raise Exception("generic fail")
        return json.loads(s)

    with patch("json.loads", side_effect=mock_loads), \
         patch("src.python.agents.dashboard_bridge.aioredis.from_url", AsyncMock(return_value=bridge.redis)):
        await bridge.forward_redis_messages()
...
async def test_bridge_broadcast_system_stats_errors(bridge):
    ...
    with patch("src.python.agents.dashboard_bridge.sqlite3.connect", side_effect=Exception("db fail")), \
         patch("src.python.agents.dashboard_bridge.asyncio.sleep", side_effect=sleep_side_effect):
        await bridge.broadcast_system_stats()
...
async def test_bridge_forward_redis_messages_poll_error(bridge):
    ...
    with patch("src.python.agents.dashboard_bridge.aioredis.from_url", AsyncMock(return_value=bridge.redis)), \
         patch("src.python.agents.dashboard_bridge.asyncio.sleep", side_effect=stop_loop):
        await bridge.forward_redis_messages()
...
async def test_bridge_broadcast_system_stats_loop_error(bridge):
    ...
    with patch.object(bridge.api_manager, "get_stats", side_effect=Exception("broadcast fail")):
        async def stop_loop(*args, **kwargs):
            bridge.running = False
            return None
        with patch("src.python.agents.dashboard_bridge.asyncio.sleep", side_effect=stop_loop):
            await bridge.broadcast_system_stats()
