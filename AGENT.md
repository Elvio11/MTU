# ⚠️ MANDATORY CONTEXT FILE ⚠️
# ALL AGENTS MUST INCLUDE THIS FILE IN THEIR CONTEXT WINDOW WHEN WORKING ON THE MTUS PROJECT
# This file contains the complete project knowledge base - refer to it before making any changes

---

# Agent.md - MTUS Complete Project Knowledge Base
# Version: 3.0.0 | Last Updated: 2026-05-08

---

# 🔴 CRITICAL INSTRUCTION FOR ALL AGENTS 🔴
**BEFORE starting any task on the MTUS project, you MUST:**
1. Read this file (`D:\Trader\Agent.md`) in full
2. Keep this file in your context throughout the conversation
3. Reference the relevant sections when making changes
4. Update this file when project structure changes

---

# MTUS (MemeTrader Unified System) - Complete Documentation
...
2. [Directory & File Manifest](#2-directory--file-manifest)
3. [Agent Deep-Dive](#3-agent-deep-dive)
4. [Core Engine Code Blocks](#4-core-engine-code-blocks)
...
9. [Dashboard Documentation](#9-dashboard-documentation)
10. [Test Suite](#10-test-suite)
11. [Appendices](#11-appendices)
12. [Project Maintenance Rules](#12-project-maintenance-rules)
...
Tests live trading preparation (follow forlive.md phases):
...
# 12. Project Maintenance Rules

## 12.1 Archive Directory Structure
All deprecated/old files are moved to `D:\Trader\archive\` (never deleted). Archive mirrors the active project structure:
```
D:\Trader\archive\
├── agents\
│   ├── python\          # Deprecated Python agents
│   └── typescript\      # Deprecated TypeScript agents
├── shared\              # Deprecated shared modules
├── tests\               # Deprecated test files
├── docs\                # Old documentation versions
├── config\              # Deprecated config files
└── scripts\             # Deprecated utility scripts
```

## 12.2 File Deletion/Archiving Protocol
**NEVER delete files from the active project directory.** Follow this workflow:
1. Identify file to remove from active directory
2. Create matching subdirectory in `D:\Trader\archive\` if it does not exist
3. Move file to corresponding archive subdirectory:
   ```bash
   # Example: Archive deprecated agent
   mv D:/Trader/src/python/agents/old_agent.py D:/Trader/archive/agents/python/
   ```
4. Update `TECHNICAL_DOCUMENTATION.md` Section 2 (Directory & File Manifest):
   - Remove file from active manifest
   - Add note: `[Archived to D:\Trader\archive\agents\python\old_agent.py]`
5. Log archiving event in `audit_ledger` PostgreSQL table

## 12.3 Missing File Recovery Protocol
If a file is missing from its expected active path:
1. **First check archive** before creating a new file:
   ```bash
   # Example: Check if missing agent exists in archive
   ls D:/Trader/archive/agents/python/ | grep "missing_agent.py"
   ```
2. If found in archive: Move file back to original active path
3. If not found in archive: Create new file following existing code/documentation formats
4. Update `TECHNICAL_DOCUMENTATION.md` Section 2 to reflect the file's status

## 12.4 Documentation Format Standards
Follow the exact structure and formatting of `TECHNICAL_DOCUMENTATION.md`:
- Use ATX-style headers (`#`, `##`, `###`)
- Use GitHub-flavored markdown tables for structured data
- Use mermaid syntax for diagrams (flowcharts, sequence diagrams)
- Use code blocks with language tags and location comments:
  ```python
  # Location: src/python/agents/nofx.py:324-470
  async def handle_pumpdev_message(self, payload: dict):
      ...
  ```
- Use callout blocks for warnings/notes:
  > **Note**: Uses `is_paper_mode()` function from `constants.py`

## 12.5 Update Triggers for Documentation
Update `TECHNICAL_DOCUMENTATION.md` immediately when:
- New agents/modules are added (update Section 2, 3)
- Test counts change (update Section 10)
- Configuration/environment variables change (update Section 5)
- Redis keys/channels are modified (update Section 5.3, 6.2)
- Agent methods/APIs are updated (update Section 3, 4)

## 12.6 Test Suite Management
### Test Creation Standards
Follow the existing test structure and naming conventions:
- Python tests: `tests/{unit|integration|e2e|chaos|security}/test_{module}.py`
- TypeScript tests: `{module}.test.ts` alongside source files
- Dashboard tests: `dashboard/src/test/` using Vitest

All tests must:
- Include descriptive docstrings
- Use fixtures for Redis/DB connections
- Assert against known good states
- Update `TECHNICAL_DOCUMENTATION.md` Section 10.1 with new test counts

### Test Update/Modification Rules
- When modifying tests: Run full test suite to verify no regressions
- When removing tests: Move to `D:\Trader\archive\tests\` (never delete)
- When adding tests: Update Section 10.2 (Test Commands) with new run commands

### Test Execution & Reporting
Run tests using commands from `TECHNICAL_DOCUMENTATION.md` Section 10.2:
```bash
# Python tests
python -m pytest --tb=short -v
...
```
After execution:
1. Verify all tests pass (target: 327+ passing tests)
2. Update Section 10.1 test counts if any tests were added/removed
3. Log test results to `audit_ledger`

## 12.7 Code & Documentation Consistency
### Naming Conventions (DO NOT CHANGE)
Enforce the conventions documented in `TECHNICAL_DOCUMENTATION.md` Section 8.3:
- Agent IDs: AGT-01 through AGT-11 (defined in `constants.py`)
- Redis keys: All use `mtus:` prefix
- Event types: Exact names (e.g., `token_gradated` not `token_graduated`)
- TypeScript shared files: Mixed conventions (lowercase, kebab-case, snake_case) as documented

### Formatting Standards
Match the formatting of `TECHNICAL_DOCUMENTATION.md`:
- Indentation: 4 spaces for code blocks, 2 spaces for nested lists
- Table alignment: Use pipe separators with consistent spacing
- Code comments: Use `# Location: <path>:<line-range>` for all code snippets
- Diagrams: Use mermaid syntax with proper subgraph labeling

## 12.8 Emergency Procedures

### Critical File Loss
If a critical file (agent, shared module, config) is missing:
1. Check `D:\Trader\archive\` corresponding subdirectory first
2. If found: Move back to active path, verify functionality
3. If not found: Restore from git history (if repo exists) or recreate following existing formats
4. Update `TECHNICAL_DOCUMENTATION.md` immediately

### Documentation Corruption
If `TECHNICAL_DOCUMENTATION.md` or `Agent.md` is corrupted:
1. Check `D:\Trader\archive\docs\` for latest backup
2. If backup exists: Restore and apply recent changes
3. If no backup: Recreate following the formatting standards in Section 12.4

---

# 13. Changelog
- [2026-05-08] **v3.0.0**: Complete rewrite - merged ALL content from TECHNICAL_DOCUMENTATION.md (all 10 sections + appendices) with maintenance rules. Added mandatory context instruction for all agents.
- [2026-05-08] **v2.0.0**: Added archiving protocols, missing file recovery, test suite management, documentation maintenance procedures
- [2026-05-05] **v1.0.0**: Initial Agent Setup Guide

---

*Document Version: 3.0.0*
*Last Updated: 2026-05-08*
*Total Test Count: 327 passing tests (261 Python + 41 TypeScript + 25 Dashboard)*
*Classification: CONFIDENTIAL*

---

# 🔴 END OF MANDATORY CONTEXT FILE 🔴
# REMINDER: ALL AGENTS MUST KEEP THIS FILE IN CONTEXT WHEN WORKING ON MTUS PROJECT
